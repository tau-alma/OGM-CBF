...
def vis_scenario_frames(scenario, name, show_x_lbl=True):
    ...
    for ti in np.arange(0,DUR+1/HZ/2,1/HZ): 
        ...
        for t in topics:
            ...
            ts = series[sc.name][t][:,0]
            plt.plot(
                    series[sc.name][t][ts < ts0+ti,0]-ts0,
                    series[sc.name][t][ts < ts0+ti,1],
                    **plot_kwargs)
        ...
        leg = ax.legend(ncols=1,loc="upper left")    
        leg.get_frame().set_alpha(0)
        ax.spines['top'].set_visible(False)
        ax.spines['right'].set_visible(False)
        plt.savefig("/tmp/%s_%05d.png" % (name,ni),
                   bbox_inches='tight', 
                   transparent=True,
                   pad_inches=0)
        plt.close()

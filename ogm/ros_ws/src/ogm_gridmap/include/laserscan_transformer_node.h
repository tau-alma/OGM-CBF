...
class TransformerNode  : public rclcpp::Node
{
  private:
    ...
    void callback_scan_registration(
        const sensor_msgs::msg::PointCloud2::ConstSharedPtr msg_scan,
        const nav_msgs::msg::Odometry::ConstSharedPtr msg_odom
        )
    {
      rclcpp::Time scan_stamp(msg_pcd->header.stamp);
      ...
      Eigen::Matrix4f T_map2link = get_T_matrix(msg_odom);     

      Eigen::Matrix4f T_map2sensor = T_map2link * T_link2sensor;
  ...
  public:
    TransformerNode() : Node("scan_transformer")
    {
      ...
      pub_pc2 = this->create_publisher<sensor_msgs::msg::PointCloud2>("pcd_transformed", 1);

      rclcpp::QoS qos = rclcpp::QoS(10);
      sub_pcd.subscribe(this, "pcd", qos.get_rmw_qos_profile());
      sub_pose.subscribe(this, "pose", qos.get_rmw_qos_profile());
      ...
      sync->registerCallback(std::bind(
          &TransformerNode::callback_pc2_registration,
          this,
          std::placeholders::_1,
          std::placeholders::_2));
...
int init_lasecan_transformer_node(int argc, char** argv)
{

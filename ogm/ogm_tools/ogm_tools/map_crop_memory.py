...
def euler_from_quaternion(q: np.array):
    ...
    returns: 
        euler (numpy array [3x1]): Euler form corresponding to given quaternion
    """
    qw, qx, qy, qz = q
    t0 = +2.0 * (qw  * qx + qy * qz)
...
class MinimalSubscriber(Node):
    ...
    def __init__(self):
        ...
        self.origin_x = 0.0 #-132.938 # from the metadata <-- does not work without in this file version
        self.origin_y = 0.0 #-122.862 # from the metadata <-- does not work without in this file version
        ...
        if self.map is None:
            print("Error: Unable to read the image from file:")
            return
        self.map_received = False
        #self.show_map()

    def map_im_callback(self, msg: Image):
        #scan recieved by lidar on mir
        self.map_received = True
        self.scan = self.cv_bridge.imgmsg_to_cv2(msg)
        
    def show_map(self):
        
        #self.map[2420:2460, 2620:2660]= (0, 255, 0)
        #show = cv2.resize(self.map, None, fx=0.2, fy=0.2, interpolation=cv2.INTER_NEAREST)
    ...
    def pose_callback(self, msg: Pose):
        ...
        if self.map_received:
            ...
            print(np.shape(self.scan))
            #target = (img + self.scan) / 2
            ...
            target[mask == 255] = [0]
            msg_img: Image = self.cv_bridge.cv2_to_imgmsg(target, encoding='8UC1')
            msg_img.header.stamp = self.get_clock().now().to_msg()
            msg_img_prev: Image = self.cv_bridge.cv2_to_imgmsg(255 - img, encoding='8UC1')
            msg_img_prev.header.stamp = self.get_clock().now().to_msg()
            #cv2.imshow("map cropped and rotated", target)
            #cv2.waitKey(1)
            print("sending images")
            self.img_pub.publish(msg_img)
            self.img_pub_comb.publish(msg_img_prev)

            # Now, rotate the combined image back into global map coordinates and integrate it.
    ...
    def integrate_combined_image(self, combined_patch, heading, current_cell):
        ...
        """
        # Define parameters based on the MAP class.
        cropped_map = MAP()  # cropped_map.width and height are 200.
        ROI_size = cropped_map.width * 2  # 400 pixels; the region initially extracted.
        
        # Step 1. Create a blank 400x400 image and place the combined_patch in its center.
        ...
        combined_full[offset:offset+combined_patch.shape[0], offset:offset+combined_patch.shape[1]] = combined_patch

        # Step 2. Recreate the rotation matrix used in rotate_image.
        center = (ROI_size // 2, ROI_size // 2)  # (200, 200)
        angle = -heading* (180.0 / np.pi) -90 #- 90#-heading * (180.0 / np.pi) -90
        T = cv2.getRotationMatrix2D(center, angle, 1.0)
        # Compute the inverse transformation.
        T_inv = cv2.invertAffineTransform(T)
        
        # Step 3. Apply the inverse warp.
        patch_before_crop = cv2.warpAffine(combined_full, T_inv, (ROI_size, ROI_size))
        
        # Step 4. Undo the horizontal flip (since rotate_image used np.fliplr).
        patch_original_ROI = np.fliplr(patch_before_crop)
        #patch_original_ROI = patch_before_crop
        #patch_original_ROI = patch_before_crop
        
        # At this point, patch_original_ROI is a 400x400 image corresponding to the ROI
        # in the original map from which the rotated patch was extracted.
        # The ROI in the global map is defined by:
        #   top_left = (current_cell_x - ROI_size//2, current_cell_y - ROI_size//2)
        global_top_left_x = int(current_cell[0] - ROI_size // 2)
        global_top_left_y = int(current_cell[1] - ROI_size // 2)
        
        # Step 5. Overlay the recovered patch onto self.map.
        # Determine the overlapping region (taking map boundaries into account).
        ...
        # Adjust indices in the patch accordingly.
        ...
        # Create a mask where the recovered_patch has non-zero values.
        mask = recovered_patch > 0
        
        # Update the corresponding region in self.map:
        map_region = self.map[roi_y_start:roi_y_end, roi_x_start:roi_x_end]
        # For pixels where mask is True, replace the value with the recovered patch.
